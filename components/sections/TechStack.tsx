...
const TechStack = () => {
  ...
  >
     <div
    className="absolute w-[150%] h-[150%] inset-0  transition-all duration-300 pointer-events-none z-[-1] group-hover:translate-x-[-10%] "
     style={{
  ...
  />
    <div>
      {typeof item.icon === 'string' ? (
        <img src={item.icon} alt={item.name} className="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] md:w-[40px] md:h-[40px] object-contain" />
      ) : (
    ...
    <p className="text-white font-medium text-center text-[10px] sm:text-[14px] md:text-[16px]">{item.name}</p>
  </div>
))}
        </div>
    </div>
